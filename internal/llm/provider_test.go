...
func (m *mockProvider) Classify(ctx context.Context, input ClassifyInput) (*ClassifyResult, error) {
	m.calledWith = append(m.calledWith, input)
	return m.result, m.err
}

func (m *mockProvider) BatchClassify(ctx context.Context, inputs []ClassifyInput) ([]*ClassifyResult, error) {
...
func TestBatchProvider_Fallback(t *testing.T) {
	...
	// Provider does NOT implement BatchProvider
	if _, ok := p.(BatchProvider); !ok {
		// It should implement it since mockProvider has BatchClassify
		// This test verifies the type assertion
	}
