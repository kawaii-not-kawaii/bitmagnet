...
def detect_codex_session(session: str, capture: str) -> str:
    agent_env = tmux_show_environment(session, "AI_AGENT")
    if agent_env == "codex":
        return "codex"
    if agent_env == "pi":
        return "pi"
    if re.search(r"(?i)OpenAI Codex|codex exec|gpt-[0-9]+-codex|tokens used|codex-cli", capture):
        return "codex"
    if re.search(r"(?i)omp --auto-approve|Oh My Pi|OMPCODE", capture):
        return "pi"
    return "claude"
...
def _runner_session_status(
    ...
    *,
    ...
    if lifecycle == "running" and child_alive:
        agent_name = str(state.get("agent") or ("codex" if codex else "claude")).capitalize()
        label = active_task or f"{agent_name} working"
        return {
            "status": "active",
    ...
    if lifecycle in {"created", "launching"} or runner_alive or pane.exists:
        agent_name = str(state.get("agent") or ("codex" if codex else "claude")).capitalize()
        label = active_task or f"launching {agent_name.lower()}"
        return {
            "status": "idle",
            "todos_done": todos_done,
            "todos_total": todos_total,
...
def _legacy_claude_session_status(
    ...
    *,
    ...
    pane_pid = _safe_int(tmux_display(session, "#{pane_pid}"))
    agent_running = pane_pid > 0 and (
        run_cmd("pgrep", "-P", str(pane_pid), "-f", "claude")[1] == 0
        or run_cmd("pgrep", "-P", str(pane_pid), "-f", "omp")[1] == 0
    )
    ...
    if activity_detected or agent_running:
        active_task = extract_active_task(capture) or "Agent working"
