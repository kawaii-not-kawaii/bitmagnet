...
def _current_skills_root() -> Path | None:
    for parent in Path(__file__).resolve().parents:
        if parent.name == STORY_SKILL_NAME and parent.parent.name == "skills":
            ...
            if skills_root.parent.name in {".agents", ".claude", ".codex", ".omp"}:
                return skills_root
    return None
...
def runtime_provider(project_root: str | Path | None = None) -> str:
    ...
    # hooks.json plus config.toml. Pi uses config.yml under ~/.omp. Marker paths are
    # resolved separately so they can follow the installed skill root in mixed or
    # migrated workspaces.
    for name in ("BMAD_RUNTIME_PROVIDER", "STORY_AUTOMATOR_RUNTIME_PROVIDER"):
        raw = os.environ.get(name, "").strip().lower()
        if raw == "omp":
            raw = "pi"
        if raw in {"claude", "codex", "pi"}:
            return raw
    ...
    if (root / ".agents" / "skills" / STORY_SKILL_NAME).exists() or (root / ".codex" / "skills" / STORY_SKILL_NAME).exists():
        return "codex"
    if (root / ".omp" / "skills" / STORY_SKILL_NAME).exists():
        return "pi"
    return "claude"
...
def active_marker_path(project_root: str | Path | None = None) -> Path:
    ...
    try:
        ...
        if skills_root.parent.name in {".claude", ".agents", ".codex", ".omp"} and (has_story_skill or explicit_root or current_root):
    ...
    if provider == "pi":
        return (root / ".omp" / ACTIVE_MARKER_NAME).resolve()
...
def resolve_portable_path(path_value: str, project_root: str | Path | None = None) -> Path | None:
    ...
    for prefix in (".claude/skills/", ".agents/skills/", ".codex/skills/", ".omp/skills/"):
