...
def ensure_stop_hook(
    *,
    ...
    if provider == "pi":
        return ensure_pi_stop_hook(project_root=project_root, command=command, timeout=timeout)
...
def ensure_pi_stop_hook(*, project_root: Path, command: str, timeout: int) -> dict[str, Any]:
    # Pi (OMP) does not use a persistent hook config file like Claude's settings.json
    # or Codex's hooks.json. Child sessions are isolated via STORY_AUTOMATOR_CHILD=true,
    # and the orchestrator's active-marker check (storiesRemaining > 0) prevents premature
    # stops without needing a CLI-level stop hook. This returns a verified status so the
    # orchestrator proceeds without attempting to write a hook file that Pi would ignore.
    return {
        "changed": False,
        "reason": "pi_stop_hook_not_required",
        "provider": "pi",
        "path": "",
        "verificationState": "verified",
        "message": "Pi (OMP) does not require a stop hook file. Child sessions are isolated via STORY_AUTOMATOR_CHILD and the active marker prevents premature stops.",
    }
